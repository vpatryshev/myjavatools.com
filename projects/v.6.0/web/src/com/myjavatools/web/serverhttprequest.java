...
public class ServerHttpRequest implements HttpServletRequest
{
  ...
  private PrintWriter logWriter = new PrintWriter(debugInfo);
  ...
  public class ComplexValue extends HashMap {
    ...
    public void extractProperty(String name, CharSequence from) {
      put(name, Strings.extractValue(from.toString(), name));
    }
  ...
  private int    MAXBUFSIZE = 1000;
  private byte[] buf = new byte[MAXBUFSIZE];
  private int    bufSize = 0;
  private int    bufIndex = MAXBUFSIZE;
  public  int    numHits = 0;
  public  int    total = 0;
  public  int    maxHits = 0;
  public  int    hitsAtThisSize = 0;
  private final boolean doAdjust = true;

  public int getByte() throws IOException {
    if (reader != null) return reader.read();
    if (bufIndex >= bufSize) {
      if (doAdjust && maxHits > 3 && maxHits > 0.6 * hitsAtThisSize) {
        maxHits = hitsAtThisSize = 0;
        MAXBUFSIZE *= 2;
        buf = new byte[MAXBUFSIZE];
        log("\n\nResizing buffer to " + MAXBUFSIZE + "\n\n");
      }

      bufIndex = 0;
      for (int attempt = 0; attempt < 3; attempt++) {
        bufSize = sis.read(buf, 0, MAXBUFSIZE);
        if (bufSize > 0) {
          break;
        }
        log("\nRead failed (" + bufSize + "), trying again.");
        try {
    ...
    if (bufSize < 1) {
      log("\n********** Oops... end of input... size is " + bufSize + "\n**************\n");
    }
    return bufIndex < bufSize ? 0xff & buf[bufIndex++] : -1;
  }
...
//    final int counterpatience = 50000;
    ...
    for (int boundaryIndex = 0; boundaryIndex < nlBoundaryBytes.length;) {
      ...
      if (read < 0) {
        log("Unexpected end of input...: " + read + "\n");
        break;
      }

      char c = (char)read;
      if (debugLevel > 6) {
        log(" \'" + Integer.toHexString(read) + ":" + c + "\' ");
      }
      ...
      if (boundaryIndex > 1) {
        if (debugLevel > 6 && boundaryIndex > 1) {
          log(" vs {" + Integer.toHexString(expected) + ":" +
      ...
      if (read != expected) {
        if (debugLevel > 6 && boundaryIndex > 1) {
          log(" (must be data, " + (boundaryIndex - 1) + " chars) ");
        }
  ...
  private String readTillBoundary() throws IOException {
    ...
    String result = new String(Bytes.toChars(baos.toByteArray()));
    if (debugLevel > 5) {
  ...
  public ServerHttpRequest(HttpServletRequest originalRequest, int debugLevel, PrintWriter printer) {
    ...
    try {
        ...
        if (contentType != null && contentType.startsWith("multipart/form-data")) {
          ...
          setBoundary();
          if (debugLevel > 1) log("reading lines...");
          ...
          while (!isEndBoundary() && readLine() != null) {
            ...
            if (value.getName() != null) {
              addValue(value);
              if (debugLevel > 1) {
                log("name is " + value.getName() + ", value is " +
                        value.toString() + "\n");
              }
            } else {
              continue;
            ...
            if (debugLevel > 1) {
              log("filename is " + value.get("filename") + "\n");
            }
            ...
            while (readLine().length() > 0) {
              ...
              log("got attribute " + attrName + " = \"" + attrValue + "\"\n");
            }
            log("End of attributes for " + value.getName() + " will read till boundary...\n");
  ...
  private void addValue(String name, ComplexValue cv) {
    ...
    if (cva != null) {
      System.arraycopy(cva, 0, newCva, 0, cva.length);
    }
  ...
  public String getParameterContentType(String name) {
    return getParameterAttribute(name, "content-type");
  }

  public String getParameterName(String parameter) {
    return getParameterAttribute(parameter, "filename");
  }
  ...
  public String[] getParameterValues(String name) {
    if (parameters == null) {
      return org.getParameterValues(name);
    }
    Object p = parameters.get(name);
    if (p == null) {
      return null;
  ...
  public int getLocalPort() {
    return org.getLocalPort();
  }
  public String getLocalAddr() {
    return org.getLocalAddr();
  }
  public String getLocalName() {
    return org.getLocalName();
  }
  public int getRemotePort() {
    return org.getRemotePort();
  }
