...
public abstract class Maps {
     ...
     *
     * @param f Map&lt;X,? extends Y> first Map
     ...
     */
    public static <X,Y,Z> Map<X,Z>
        compose(Map<X,? extends Y> f, Map<Y,Z> g) {
      return compose(f, forMap(g));
  }
  /**
   * Composes a Map and a Function (see Functions#compose) by applying function
   * to all the values in the map.
   *
   * @param f Map&lt;X,? extends Y>  Map
   * @param g Function&lt;Y,Z> Function
   * @return Map&lt;X,Z> composition, x -> g(f(x));
   *
   * <br><br><b>Example</b>:
   * <p>Suppose we have the following: <br>
   * <code>Map&lt;Integer,String> f = toMap(new Object[] {1, "one", 2, "two", 3, "three"});<br>
   *       Function&lt;String,String> g ... { return toUppper(string); }...</code>
   * <p>Then <code>compose(f, g)</code><br>
   * returns the same map as produced by <br>
   * <code>toMap(new Object[] {1, "ONE", 2, "TWO", 3, "THREE"});</code><br>
   */
  public static <X,Y,Z> Map<X,Z>
      compose(Map<X,? extends Y> f, Function<Y,Z> g) {
    return Function.compose(forMap(f), g).toMap(f.keySet());
  }
   ...
   * @param map Map&lt;X,Y>
   * @param y Y - the key
   * @return Iterable&lt;X> - all X x such that y.equals(f(x)).
   *
   * If you want to obtain a Set instead of an Iterable, please use
   * resolveToSet().
   */
  public static <X,Y> Iterable<X> resolve(final Map<X,Y> map, final Y y) {
    return new Filter<X>() {
      public boolean accept(X x) {
        return y.equals(map.get(x));
      }
    }.filter(map.keySet());
  }
   ...
   * @return Set
   */
  public static <X,Y> Set<X> resolveToSet(Map<X,Y> map, Y y) {
    Set<X> result = new LinkedHashSet<X>();
    Objects.addAll(result, resolve(map, y));
    return result;
  }
   ...
   * @param domain List&lt;? extends X> the domain list
   ...
   */
  public static <X,Y> List<Y> map(Map<X,Y> m, List<? extends X> domain) {
    return forMap(m).apply(domain);
  }
   ...
   *
   * @param map Map&lt;X,Y>
   * @param keys Set&lt;? extends X> of keys
   * @return Map&lt;X,Y>
   */
  public static <X,Y> Map<X,Y> restrict(final Map<X,Y> map,
                                        final Set<? extends X> keys) {
   ...
   *
   * Resulting Map is a virtual map that has an intersection of
   * map's keyset and the set of keys from collection as a keyset, and maps them
   * to the same values as the original map does.
   *
   * @param map Map
   ...
   */
  public static <X,Y> Map<X,Y> restrict(final Map<X,Y> map,
                                        final Collection<? extends X> keys) {
    return new AbstractMap<X,Y>() {
      public Set<Map.Entry<X,Y>> entrySet() {
        return new RestrictedMapEntrySet<X,Y>(map, keys);
      }
   ...
   *
   * @param m Map&lt;X,Y> the map
   * @param domain Colection&lt;? extends X> the domain list
   ...
   * m.put("a", "ValueOfA"); m.put("b", "ValueOfB"); m.put("c", "ValueOfC");</code>
   * <p>Then <code>map(m, Arrays.asList(new String[] {"b", "x", "b"}))</code>
   * returns a Collection that contains {"ValueOfB", null, "ValueOfB"}.
   */
  public static <X,Y> Collection<Y> map(final Map<X,Y> m,
                                        final Collection<? extends X> domain) {
   ...
   * If Map m is considered as a function from its keys to its values, then
   * the iterator that map returns is the one that,
   * for each x in iterator, returns m(x) is returned by next() call.
   *
   * @param m Map&lt;X,Y> the map
   * @param iterator Iterator&lt;? extends X> the domain iterator
   ...
   */
  public static <X,Y> Iterator<Y> map(Map<X,Y> m,
                                      Iterator<? extends X> iterator) {
    return forMap(m).apply(iterator);
  }
   ...
   *
   * @param m Map&lt;X,Y> the map
   * @param iterable Iterable&lt;? extends X> the domain iterator
   ...
   */
  public static <X,Y> Iterable<Y> map(Map<X,Y> m,
                                      Iterable<? extends X> iterable) {
    return forMap(m).apply(iterable);
  }
   ...
   * returns "deux";</li>
   */
  public static <X,Y> Map<X,Y> toMap(Map.Entry<X,Y>... pairs) {
   ...
   *
   * @param keysAndValues Object... odd elements of the array are keys, and even elements are values
   ...
   */
  public static <X,Y> Map<X,Y> toMap(Object... keysAndValues) {
      return new KeyValueArrayMap<X,Y>(keysAndValues);
  }
