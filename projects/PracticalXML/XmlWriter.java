...
public class XmlWriter extends Writer
{
  ...
  static private boolean needsEscape(char c) {
 ...
 */
  ...
  private void writeXmlData(BasicXmlData x)
    throws IOException {

    Map a = x.getAttributes();
    Collection kids = x.getAllKids();

    if ((kids != null) && (kids.size() > 0)) {
      openElement(x.getType(), a);

      for (Iterator i = kids.iterator(); i.hasNext();) {
        eol();
        writeXmlData((BasicXmlData)i.next());
      }
      if (!isAlmostEmpty(x.getValue())) {
        write(xmlEscape(x.getValue()));
      } else {
        eol();
      }
      closeElement();
    } else {
      writeElement(x.getType(), x.getValue(), a);
    }
  }
...
*/
   ...
   *
   * @param o the data to be sent to output. Can be XmlData or a String.
   * @throws IOException
   */
...
//      writeMap((Map)o);
  ...
  private static final boolean isAlmostEmpty(String string) {
