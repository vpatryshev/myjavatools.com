...
public abstract class Function<X,Y> {
   ...
   *
   * @param x Set&lt;X> the set of map keys
   * @return Map&lt;X,Y> that maps keys from X to function values on those keys
   */
  public Map<X,Y> toMap(final Set<X> x) {
    return new FunctionalMap<X,Y>(this, x);
  }

  /**
   * Restricts this function to a specified Collection, returning a map on the set of
   * collection entries
   * @param x Collection&lt;X> keys
   * @return Map&lt;X,Y> that maps keys from Collection to function values on those keys
   */
  public Map<X,Y>
      toMap(final Collection<X> x) {
    return new AbstractMap<X,Y>() {
      public Set<Map.Entry<X,Y>> entrySet() {
        return new RestrictedMapEntrySet<X,Y>(this, x);
      }
    };
  }
   ...
   */
  public static <X,Y> Function<X,Y> function(final Map<X,Y> map) {
   ...
   * @return List
   */
  public List<Y> apply(List<X> domain) {
    return new FunctionValueList<X,Y>(this, domain);
  }
   ...
   */
  public Iterator<Y> apply(final Iterator<X> iterator) {
//    return Iterators.map(this, iterator);
   ...
   */
  public Iterable<Y> apply(final Iterable<X> iterable) {
   ...
   */
  public <W> Function<W,Y> compose(final Function<W,X> g) {
    final Function<X,Y> f = this;
    return new Function<W,Y> () {
      public Y apply(W w) {
        return Function.this.apply(g.apply(w));
      }
    };
  }
  /**
