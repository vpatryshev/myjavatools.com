...
//    xmlData.setAttributes(attributes);
  ...
  public void testRemoveKid_wasPresent() {
    XmlData modified = (XmlData)xmlData.clone();
    XmlData kid = modified.getKid("person");
    modified.removeKid(kid);
    assertFalse("kid removed, must be different", modified.equals(xmlData));
  }

  public void testRemoveKid_wasAbsent() {
    XmlData modified = (XmlData)xmlData.clone();
    XmlData kid = new BasicXmlData("nonperson", "Absolutely Unimportant One");
    modified.removeKid(kid);
    assertEquals("nothing was removed, no change expected", xmlData, modified);
  }
