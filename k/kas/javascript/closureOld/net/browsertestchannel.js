 ...
 *
 * @constructor
 * @param {BrowserChannel} channel  The BrowserChannel that owns this test
 *                                  channel.
 */
goog.net.BrowserTestChannel = function(channel) {
  /**
 ...
 * @private
 */
goog.net.BrowserTestChannel.State_.INIT = 0;
 ...
 * @private
 */
goog.net.BrowserTestChannel.State_.CHECKING_BLOCKED = 1;
 ...
 * @private
 */
goog.net.BrowserTestChannel.State_.CONNECTION_TESTING = 2;
...
goog.net.BrowserTestChannel.prototype.connect = function(path) {
  ...
  this.request_ = new goog.net.ChannelRequest(this);
 ...
 * feature that allows network administrators to block Yellow Jacket. The
 ...
 * are trying to block chat. For YJ, the subdomain is
...
goog.net.BrowserTestChannel.prototype.checkBlocked_ = function() {
  ...
  goog.net.testLoadImageWithRetries(uri,
      goog.net.BrowserTestChannel.BLOCKED_TIMEOUT_,
      this.checkBlockedCallback_.bind(this),
 ...
 * @private
 */
goog.net.BrowserTestChannel.prototype.checkBlockedCallback_ =
    function(succeeded) {
  if (succeeded) {
...
goog.net.BrowserTestChannel.prototype.connectStage2_ = function() {
  goog.net.ChannelDebug.debug('TestConnection: starting stage 2');
  this.request_ = new goog.net.ChannelRequest(this);
  this.request_.setMaxRetries(0);
...
goog.net.BrowserTestChannel.prototype.onRequestData =
  ...
  if (this.state_ == goog.net.BrowserTestChannel.State_.INIT) {
    goog.net.ChannelDebug.debug('TestConnection: Got data for stage 1');
    if (!responseText) {
      goog.net.ChannelDebug.debug('TestConnection: Null responseText');
...
goog.net.BrowserTestChannel.prototype.onRequestComplete =
  ...
  if (!this.request_.getSuccess()) {
    goog.net.ChannelDebug.debug('TestConnection: request failed, in state ' +
                                this.state_);
    if (this.state_ == goog.net.BrowserTestChannel.State_.INIT) {
      goog.net.BrowserChannel.notifyStatEvent(
  ...
  if (this.state_ == goog.net.BrowserTestChannel.State_.INIT) {
    goog.net.ChannelDebug.debug('TestConnection: request complete for ' +
                                'initial check');
    ...
    if (goodConn) {
      goog.net.ChannelDebug.debug('Test connection succeeded; using ' +
                                  'streaming connection');
      goog.net.BrowserChannel.notifyStatEvent(
          goog.net.BrowserChannel.Stat.NOPROXY);
      this.channel_.testConnectionFinished(this, true);
    } else {
      goog.net.ChannelDebug.debug('Test connection failed; not using ' +
                                  'streaming');
      goog.net.BrowserChannel.notifyStatEvent(
